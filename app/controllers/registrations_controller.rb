...
def create
  ...
  if !::Milia.use_recaptcha  ||  verify_recaptcha
    ...
    Tenant.transaction  do 
      ...
      if flash[:error].blank? || flash[:error].empty? #payment successful
        ...
        if resource.errors.empty? # SUCCESS!
          ...
          raise ActiveRecord::Rollback # force the tenant transaction tobe rolled back
    ...
    flash[:error] = "Recapcha codes didn't match; please try again"
    #all validation erros are passed when the sign-up form is re-rendered
    resouce.valid?
    @tenant.valid?
    log_action("recapcha failed", resource)
    render :new and return 
  end
end #def create
